...
export function RecentIncidents({ data = [] }: { data: any[] }) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {data.length === 0 ? (
          <p className="text-sm text-gray-500">No recent incidents found.</p>
        ) : (
          data.slice(0, 3).map((incident) => (
            <div key={incident.id} className="border-l-4 border-red-300 pl-4 py-3 bg-red-50 rounded-r-lg">
              <div className="flex justify-between items-start mb-2">
                ...
                <Badge variant={getSeverityColor(incident.priority)} className="text-xs">
                  {incident.priority}
                </Badge>
              </div>
