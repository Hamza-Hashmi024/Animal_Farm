...
export function SlaughterRecords() {
  ...
  try {
    // ✅ Directly get the array
    const recordsRaw = await ViewRecentSlaughterRecords();

    const records: SlaughterRecord[] = recordsRaw.map((record: any) => ({
      ...record,
      weightBeforeSlaughter: parseFloat(record.weightBeforeSlaughter) || 0,
      finalWeightGain: parseFloat(record.finalWeightGain) || 0,
      carcassWeight: parseFloat(record.carcassWeight) || 0,
      carcassRatio: parseFloat(record.carcassRatio) || 0,
      carcassQuality: record.carcassQuality?.trim() || "",
    }));
        ...
        <CardContent>
          <div className="space-y-4">
            {slaughterRecords.length === 0 ? (
              <p className="text-sm text-gray-500">No recent slaughter records found.</p>
            ) : (
              slaughterRecords.slice(0, 4).map((record) => (
                <div key={record.id} className="border rounded-lg p-4 space-y-3">
                  ...
                  {record.customerFeedback && (
                    <div className="text-sm text-gray-600 italic">"{record.customerFeedback}"</div>
                  )}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {["A", "B", "C"].map((grade) => {
                  const count = grade === "A" ? gradeACount : grade === "B" ? gradeBCount : gradeCCount;
                  const percentage = getPercentage(count);
                  const color =
                    grade === "A" ? "bg-green-500" : grade === "B" ? "bg-blue-500" : "bg-amber-500";
                  return (
