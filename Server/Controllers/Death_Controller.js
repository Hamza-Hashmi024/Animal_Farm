...
const addDeathRecord = (req, res) => {
  ...
  db.query(getAnimalQuery, [animalId], (err, results) => {
    ...
    db.query(
      insertDeathQuery,
      [animalId, cause, causeOfDeath, date, location],
      (err, result) => {
        if (err) {
          console.error("Error inserting death record:", err);
          return res
            .status(500)
            .json({ message: "Failed to save death record" });
        }
        ...
        db.query(updateStatusQuery, [animalId], (err) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({
                message: "Death recorded, but failed to update animal status",
              });
          }
            ...
            (err) => {
              if (err) {
                ...
                return res
                  .status(500)
                  .json({
                    message: "Status updated, but history not recorded",
                  });
              }
              ...
              res.status(200).json({
                message:
                  "Death record, status update, and history log completed successfully",
                farm,
                pen,
                deathId,
              });
...
const getAnimalById = (req, res) => {
  ...
  const query =
    "SELECT farm, pen FROM animals WHERE tag = ? AND status = 'Active'";
...
const getAllDeathRecord = (req, res) => {
  const query = "SELECT * FROM death_records";

  db.query(query, (err, results) => {
    if (err) {
      console.error("DB Error while fetching death records:", err);
      return res.status(500).json({ message: "Database error" });
    }
